...
module Words =
    ...
    let event =
        function
        | Ran(ran, reached, living) -> $"Ran {generations ran} to generation {reached}, and {cells living} alive."
    ...
    let rejection =
        function
        | NoSuchCell said -> $"There is no cell {cell said}. {Grid.span Torus.grid}"
