...
module Render =
    ...
    let heading beholder play =
        ...
        | Mustering place ->
            ...
            $"The muster - {seated yours (Seat.at place)} to place, {Words.units left} still to muster"
    ...
    let private mustering beholder play =
        ...
        [ Scene.says $"{Words.units (Squad.Strong - Squad.mustered squad)} still to muster."
          ...
          Scene.quietly $"{Squad.Strong} to a squad, and {Squad.Alike} of a kind at the most." ]
    ...
    let help =
        String.concat
            ...
            [ Scene.prose
              ...
              "THE MUSTER"
              Scene.prose
                  $"{Words.units Squad.Strong} each, taken from the roster, at most {Squad.Alike} of a kind, placed a unit at a time turn and turn about. Neither squad sees the other's until both are on the field."
              ""
              "THE RANKS"
              Scene.prose Notes.ranks
              ""
              "THE GROUND"
              Scene.prose
              ...
              ""
              Scene.prose
                  $"A blow steps aside once and no further - nothing steps in front of a blow aimed at a warder. A squad with nobody left up is broken, and the other holds the field. If neither breaks in {Words.rounds Session.Rounds}, the squad with more left standing takes it."
              ""
              Scene.prose
...
"""
    ...
    let shell =
        { Title = "Warband"
          ...
          // Space for the thing that runs, as Life's page has it: a page has no hold of its own,
          // so the key is free there where a terminal's belongs to the table. A full stop is one
          // blow at a time.
          Keys = [ " ", "run"; "p", "run"; ".", "step" ] }
