...
module Words =
    ...
    /// What a kind does from a rank, at the width a table of six of them has to fit in.
    let briefly =
        function
        | Strikes(power, 1) -> $"strike {power}"
        | Strikes(power, times) -> $"strike {power} x{times}"
        | Shoots(power, 1) -> $"shoot {power}"
        | Shoots(power, times) -> $"shoot {power} x{times}"
    ...
    let atLength =
        function
        | Strikes(power, times) ->
            $"{blows times} of {power}, hand to hand: they fall on the foremost rank of the other squad that still has anybody up, on whoever there has the most left in them."
        | Shoots(power, times) ->
            $"{blows times} of {power}, shot over the ranks: they ignore where somebody is standing and find whoever is nearest to falling."
        | Mends power ->
            $"Mends {power} back into whichever of the hexes it touches is missing the most. It cannot bring anybody back up."
