...
let private start () =
    Update.start 2 42UL |> Result.toOption |> Option.get

/// Some colour the player to act is holding.
let private held model =
    (Game.active (Model.game model)).Bag |> Pile.toColors |> List.head
...
let after = Update.update move before
...
report
    ...
    (Model.session (start ()))
    (Model.session (send (List.replicate 20 Undo) after))
...
let negotiated = Update.update (Make Negotiate) (playOn 1 (start ()))
...
report "taking a move back is itself written down" 8 (recorded (Update.update Undo (playOn 7 (start ()))))
...
let replayed =
    Update.replay (Model.players walked) (Model.seed walked) (Journal.moves walked.Journal)
    |> Result.toOption
    |> Option.get

report "playing the record again ends where it left off" (Model.session walked) (Model.session replayed)
...
let fromFile =
    Update.replay read.Players read.Seed read.Moves |> Result.toOption |> Option.get

report "a game read back from its file is the same game" (Model.session walked) (Model.session fromFile)
