...
// A house writes real records into logs/ as it is played - that is what a house is for - so every
// record this file makes is stamped where no real one could be, and taken away again at the end
// whatever happened in between. One left behind is a record CI replays on every run.
let private ours () =
...
let private screenFor console posts =
    ...
    |> List.tryPick (fun post ->
        ...
        | at, ToPlayer.Screen text when at = console -> Some text
        | _ -> None)
    |> Option.defaultValue ""
...
/// How long a check waits for the broom to have swept: as long as it takes, up to this, rather
/// than a sleep guessed at.
let private patience = TimeSpan.FromSeconds 5.0
...
try
    ...
    report
        "the ways it can be played are offered by name, for the form that asks"
        [ "turncoats" ]
        ((hosting ()).Ways |> List.map fst)
    ...
    report
        "a table somebody took a seat at is never swept, however long it waits"
        false
        (spent long (standing Lobby.Filling 2 0 1 0))

    report
        "and a game being played is never swept, however long a turn takes"
        false
        (spent long (standing Lobby.Underway 2 0 2 0))
    ...
    report
        "a table taken up off a record has a game in it, and is kept however long nobody comes back to it"
        false
        (spent
            long
            { standing Lobby.Filling 2 0 0 0 with
                Begun = true })
    ...
    report "a new house holds nothing" 0 (List.length (house ()).Listed)
    ...
    // A sweep measures from when the house saw a table's stage change, not from when the table was
    // opened: a game that finishes on Thursday was not finished on Monday.
    report
        "a game that finishes long after it was opened is kept a day from finishing, not from opening"
        (Lobby.Finished, [], [ "table-1" ])
        (let it = house ()

         match it.Opens(twoPeople, Some 42UL, None) with
         | Error said -> failwith said
         | Ok opened ->
             clock.Value <- at (60.0 * 24.0 * 3.0)
             full opened.Table |> ignore

             for who, line in [ "one", "negotiate"; "one", "return r"; "two", "negotiate"; "two", "return r" ] do
                 opened.Table.Said(who, line) |> ignore

             opened.Table.Left "one" |> ignore
             opened.Table.Left "two" |> ignore
             let stage = opened.Table.Standing.Stage
             it.Listed |> ignore
             clock.Value <- at (60.0 * 24.0 * 3.0 + 60.0)
             let early = it.Swept()
             clock.Value <- at (60.0 * 24.0 * 4.0 + 60.0)
             stage, early, it.Swept())

    report
        "a table taken up from a record outlives its first hour with nobody back at it, being somebody's game"
        ([], 1)
        (match leftBehind () with
         | [ path ] ->
             let it = house ()

             match it.Resumes path with
             | Error said -> failwith said
             | Ok _ ->
                 clock.Value <- at (60.0 * 24.0)
                 it.Swept(), List.length it.Listed
    ...
    // The clocks behind `Sweeping` and `Beating` are set again only once a tick has finished, so two
    // never overlap, and a tick that throws is said rather than swallowed. Neither is checked here:
    // a check would need a tick to outlast its interval or to throw, and both are `Clock.ticking`'s
    // alone to keep.
    // The broom says what it swept, and that is the signal a check waits on: something happening
    // is heard the moment it happens rather than guessed at with a sleep.
    report
        ...
        (let it = impatient ()
         ...
         use sweptOnce = new ManualResetEventSlim(false)
         use _ = it.Sweeping(quickly, (fun _ -> sweptOnce.Set()))
         sweptOnce.Wait patience |> ignore
    ...
    report
        ...
        (let it = impatient ()
         ...
         use told = new ManualResetEventSlim(false)
         ...
         told.Wait patience |> ignore

         heard.Count >= 1)

    // Nothing happening is the one thing no signal can say, so this one waits out fifteen sweeps.
    report
