...
// The two of these the contract never reaches - a table waiting on a stone, and an answer with a
// ruling's working in it - held as every fragment is. The rest are held for every game in `Conforms`.
Conforms.patched (
    screens
    |> List.filter (fun (name, _, _) -> name = "board, mid-negotiation" || name = "the working behind a ruling")
)
...
let private lines =
    screens |> List.collect (fun (_, _, markup) -> Conforms.posted markup)
...
report
    ...
    true
    (Conforms.posted (view.Board Margins.all (Game.active (Playing.game owing)).Id owing)
...
let private offered =
    Conforms.posted (view.Board Margins.all beholder dealt)
